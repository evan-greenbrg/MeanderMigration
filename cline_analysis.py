...
Note: The dpcore package is derived from: https://github.com/dpwe/dp_python.
      This specific package is poorly supported now with the last update 9 years ago (as of 2023).
      It only works on Python 2, which can be troublesome. The dp_python package is included
      in its entirety in this repository.
"""
import numpy as np
import matplotlib
...
def correlate_clines(x1,x2,y1,y2,penalty):
    ...
    p,q,C,phi = dpcore.dp(sm,penalty=penalty,gutter=0.0)
    return p,q,sm
...
def get_migr_rate(x1,y1,x2,y2,years,penalty):
    ...
    return migr_rate, migr_sign, pn, qn
